    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 text-slate-900 md:p-8">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-[0.18em] text-emerald-700">
              Đoàn ghép · lịch #{tour.departureId}
            </div>
            <h1 className="mt-2 text-3xl font-black tracking-tight">{tour.tourName}</h1>
            <p className="mt-1 text-sm font-semibold text-slate-500">
              Quản lý một đoàn theo cùng lịch khởi hành, thay vì xử lý từng đơn rời rạc.
            </p>
          </div>
        ...
        {tracking?.needsReviewBookings > 0 && (
          <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <b>Có báo cáo hành trình cần xem lại</b>
                <p className="mt-1">Nhân viên đã ghi nhận thay đổi hoặc bỏ qua hoạt động trong đoàn.</p>
              </div>
            </div>
            <a
              href="#tracking"
              className="inline-flex h-10 items-center justify-center rounded-xl bg-amber-700 px-4 text-xs font-black text-white hover:bg-slate-950"
            >
              Xem theo dõi
            </a>
          </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <SummaryCard
            icon={UsersRound}
            label="Khách đã giữ chỗ"
            value={`${tour.occupiedPeople || 0}/${tour.maxPeople || 0}`}
            detail={`Còn ${tour.availableSeats || 0} chỗ trống`}
          />
          <SummaryCard
            icon={ClipboardList}
            label="Đơn trong đoàn"
            value={tour.bookingCount || 0}
            detail={`${tour.pendingBookingCount || 0} chờ · ${tour.confirmedBookingCount || 0} xác nhận`}
          />
          <SummaryCard
            icon={UserRound}
            label="Nhân viên phụ trách"
            value={tour.assignedStaffName || "Chưa phân công"}
            detail={tour.assignedStaffEmail || "Cần phân công trước khi chốt đoàn"}
            compact
          />
          <SummaryCard
            icon={Activity}
            ...
            value={`${progress}%`}
            detail={`${tracking?.completedActivities || 0}/${tracking?.totalActivities || 0} hoạt động hoàn thành`}
          />
        </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <main className="space-y-6">
            <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Thông tin chuyến</p>
                    <h2 className="mt-1 text-2xl font-black">{tour.tourName}</h2>
                  </div>
                  <span className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-black text-slate-700">
                    ...
                    {formatDate(tour.departureDate)} · {formatTime(tour.departureTime)}
                  </span>
                </div>
              </div>
              <div className="p-5">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="font-black">Mức độ lấp đầy đoàn</div>
                    <p className="mt-1 text-sm font-semibold text-slate-500">
                      Số ghế đã được giữ theo tất cả đơn cùng lịch khởi hành.
                    </p>
                  </div>
                  <b className="text-2xl text-emerald-700">{capacityPercent}%</b>
                </div>
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-100">
                  <div className="h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${capacityPercent}%` }} />
                </div>
              </div>
            </section>
            ...
            <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-5">
                <div>
                  <h2 className="text-xl font-black">Các đơn trong đoàn</h2>
                  <p className="mt-1 text-sm font-semibold text-slate-500">Danh sách khách cùng tham gia chuyến này.</p>
                </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {bookings.map((booking) => (
                      <tr key={booking.id} className="align-top transition hover:bg-slate-50">
                        <td data-label="Mã đơn" className="px-5 py-4">
                          <div className="font-black text-slate-950">{booking.bookingCode}</div>
                          <div className="mt-1 text-xs font-semibold text-slate-500">#{booking.id}</div>
                        </td>
                        ...
                        <td data-label="Số khách" className="px-5 py-4">
                          <div className="font-black">{booking.totalPeople || 0} khách</div>
          ...
          <aside className="space-y-6">
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-xl font-black">Điều hành đoàn</h2>
              <p className="mt-1 text-sm font-semibold leading-6 text-slate-500">
                Thao tác tại đây áp dụng cho toàn bộ booking trong cùng lịch khởi hành.
              </p>
              <div className="mt-5 space-y-3">
                {operationSteps.map(([number, title, description], index) => (
                  <div
                    key={number}
                    className={`flex gap-3 rounded-2xl border p-3 ${
                      index === 1 && !tour.assignedStaffId
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-black">Phân công & chốt đoàn</h2>
              </div>
              ...
              <select
                value={staffId}
                disabled={busy || tour.groupStatus === "COMPLETED"}
                onChange={(event) => setStaffId(event.target.value)}
                className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 font-bold outline-none focus:border-emerald-500 focus:bg-white disabled:opacity-60"
              >
              ...
              <button
                type="button"
                disabled={busy || !staffId || tour.groupStatus === "COMPLETED"}
                onClick={saveStaff}
                className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-emerald-300 bg-white px-4 text-sm font-black text-emerald-800 transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <UserRound size={16} /> Lưu phân công
              </button>
              <button
                type="button"
                disabled={busy || !canConfirm}
                onClick={confirmGroup}
                className="mt-2 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 text-sm font-black text-white transition hover:bg-slate-950 disabled:cursor-not-allowed disabled:bg-slate-300"
              >
                <CheckCircle2 size={17} /> Xác nhận đoàn
              </button>
              <p className="mt-3 text-xs font-semibold leading-5 text-slate-500">
                {isConfirmed
                  ? "Đoàn đã chốt; lịch khởi hành không nhận thêm khách."
                  : !tour.assignedStaffId
                  ? "Hãy phân công nhân viên trước khi xác nhận đoàn."
                  : !tour.pendingBookingCount
                  ? "Không còn đơn chờ xác nhận trong đoàn."
                  : "Xác nhận sẽ chốt toàn bộ đơn chờ và đóng nhận khách cho lịch này."}
              </p>
            </section>
          </aside>
        </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="flex items-center gap-2 text-xl font-black">
                ...
                Theo dõi hành trình đoàn
              </h2>
              <p className="mt-1 text-sm font-semibold text-slate-500">Tổng hợp tiến độ từ các booking trong đoàn.</p>
            </div>
            <span className="rounded-full bg-emerald-50 px-3 py-2 text-sm font-black text-emerald-800">
              Tiến độ chung: {progress}%
            </span>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <SummaryCard label="Đơn có lịch trình" value={tracking?.bookingsWithSchedule || 0} detail="Đã tạo timeline" />
            <SummaryCard label="Đơn hoàn thành" value={tracking?.completedBookings || 0} detail="Đủ điều kiện kết thúc" />
            <SummaryCard label="Cần xem báo cáo" value={tracking?.needsReviewBookings || 0} detail="Có thay đổi/bỏ qua" />
            <SummaryCard
              label="Hoạt động hoàn thành"
              value={`${tracking?.completedActivities || 0}/${tracking?.totalActivities || 0}`}
              detail="Tổng các lịch trình"
            />
          </div>
        </section>
...
function SummaryCard({ icon: Icon, label, value, detail, compact = false }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      {Icon && <Icon size={21} className="text-emerald-700" />}
      <div className={`mt-3 font-black tracking-tight ${compact ? "text-xl" : "text-3xl"}`}>{value}</div>
      <div className="mt-1 text-sm font-bold text-slate-500">{label}</div>
      {detail && <div className="mt-2 text-xs font-semibold text-slate-400">{detail}</div>}
    </div>
  );
}

function GroupScheduleReports({ days }) {
  const reportedActivities = days
    .flatMap((day) => (day.activities || []).map((activity) => ({ ...activity, day })))
    .filter((activity) => activity.actualNote || activity.attachmentUrl || activity.updatedByEmployeeName);
  ...
  if (!days.length) {
    return (
      <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm font-semibold text-slate-500">
        Chưa có lịch trình để nhân viên cập nhật.
      </div>
    );
  }
  ...
  if (!reportedActivities.length) {
    return (
      <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm font-semibold text-slate-500">
        Nhân viên chưa gửi ghi chú hoặc ảnh báo cáo. Báo cáo sẽ xuất hiện tại đây ngay sau khi được lưu.
      </div>
    );
  }
  ...
  return (
    <div className="mt-5 border-t border-slate-100 pt-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-black">Báo cáo từ nhân viên</h3>
          <p className="mt-1 text-sm font-semibold text-slate-500">
            Ghi chú và ảnh minh chứng được gửi trực tiếp từ timeline của nhân viên.
          </p>
        </div>
        <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-800">
          {reportedActivities.length} báo cáo
        </span>
      </div>
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {reportedActivities.map((activity) => (
          <article key={activity.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 bg-white p-4">
              <div>
                <div className="text-xs font-black uppercase tracking-wider text-slate-400">
                  Ngày {activity.day.dayNumber} · {formatDate(activity.day.scheduleDate)}
                </div>
                <h4 className="mt-1 font-black text-slate-950">{activity.title}</h4>
              </div>
              <Badge meta={getMeta(scheduleActivityStatusMeta, activity.status)} />
            </div>
            <div className="p-4">
              {activity.actualNote && <p className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{activity.actualNote}</p>}
              ...
              {activity.attachmentUrl && (
                <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block px-3 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-50"
                  >
                    Xem ảnh / tệp minh chứng
                  </a>
                </div>
              )}
            </div>
