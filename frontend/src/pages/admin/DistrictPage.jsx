...
import {
  ...
  MapPinned, ChevronLeft, ChevronRight, Loader2, ArrowUpAZ,
} from "lucide-react";
...
export default function DistrictPage() {
  ...
  }, [direction, page, search, selectedProvince, sortBy]);
  ...
  useEffect(() => {
    setPage(0);
  }, [search, selectedProvince, sortBy, direction]);
  ...
  const handleSave = async () => {
    const newErrors = {};
    if (!name.trim()) newErrors.name = "Tên quận huyện không được để trống";
    else if (name.trim().length < 2) newErrors.name = "Tên quận huyện phải có ít nhất 2 ký tự";
    ...
    try {
      setLoading(true);
      const payload = { name, provinceId };
      if (editId) {
        await updateDistrict(editId, payload);
        toast.success("Cập nhật quận huyện thành công!", { icon: "🗺️" });
      } else {
        await createDistrict(payload);
        toast.success("Thêm quận huyện thành công!", { icon: "🏔️" });
      }
      setName("");
      setProvinceId("");
      setEditId(null);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Quận huyện</h2>
              <p className="text-slate-500 font-medium italic">Hệ thống quản lý địa phương vùng cao</p>
            </div>
          </div>

          <button
            onClick={() => { setOpen(true); setEditId(null); setName(""); setProvinceId(""); setErrors({}); }}
          ...
          >
            <Plus size={20} />
            <span className="font-bold">Thêm quận huyện</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-8">
          ...
          {/* FILTER BOX */}
          <div className="lg:col-span-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-100 flex flex-col xl:flex-row items-center gap-2">
            {/* SEARCH */}
            ...
            <select
              ...
              className="w-full xl:w-52 px-4 py-3 rounded-xl bg-slate-50 outline-none border-2 border-transparent focus:border-amber-500 text-sm"
            >
            ...
            <div className="relative w-full xl:w-52">
              <ArrowUpAZ className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value)}
                className="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-50 outline-none border-2 border-transparent focus:border-amber-500 text-sm appearance-none"
              >
                <option value="name">Sắp xếp theo quận huyện</option>
            ...
            <select
              ...
              className="w-full xl:w-32 px-4 py-3 rounded-xl bg-slate-50 outline-none border-2 border-transparent focus:border-amber-500 text-sm"
            >
            ...
            <table className="admin-responsive-table admin-medium-table w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50">
                  ...
                  <th className="px-8 py-5 text-[13px] font-bold text-slate-400 uppercase tracking-widest">Quận huyện</th>
                  <th className="px-8 py-5 text-[13px] font-bold text-slate-400 uppercase tracking-widest">Tỉnh thành</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {tableLoading ? (
                  ...
                  data.map((d, index) => (
                    <tr key={d.id} className="group hover:bg-slate-50/80 transition-all duration-300">
                      ...
                      <td data-label="Thao tác" className="px-8 py-5">
                        <div className="flex justify-end gap-2 transition-opacity duration-300">
                          <button
                            onClick={() => { setEditId(d.id); setName(d.name); setProvinceId(d.provinceId); setOpen(true); }}
        ...
        <Modal open={open} onClose={() => { setOpen(false); setErrors({}); }}>
          <div className="p-2">
            <h3 className="text-2xl font-black text-slate-800 mb-2">
              {editId ? "Cập nhật quận huyện" : "Thêm mới quận huyện"}
            </h3>
            <p className="text-sm text-slate-500 mb-8 font-medium">Vui lòng nhập thông tin chính xác.</p>

            <div className="space-y-6">
              {/* NAME */}
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">Tên quận huyện</label>
                <input
                  value={name}
              ...
              {/* PROVINCE */}
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">Tỉnh thành</label>
                <select
                  value={provinceId}
                  onChange={(e) => { setProvinceId(e.target.value); if (errors.provinceId) setErrors({ ...errors, provinceId: "" }); }}
                  className={`w-full h-12 px-5 rounded-xl bg-slate-50 outline-none border-2 transition-all ${
                    errors.provinceId ? "border-rose-400 bg-rose-50" : "border-transparent focus:border-emerald-600 focus:bg-white"
                  }`}
                >
                ...
                {errors.provinceId && <p className="text-rose-500 text-xs mt-2 font-bold ml-2 italic">{errors.provinceId}</p>}
              </div>
