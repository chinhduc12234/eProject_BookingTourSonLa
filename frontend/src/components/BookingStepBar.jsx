    ...
    <div className="booking-flow-panel rounded-2xl border border-white/10 bg-[#07120f]/88 p-3 shadow-soft-dark backdrop-blur-xl">
      <div className="grid gap-2 md:grid-cols-4">
        {steps.map((step, index) => {
          ...
          const className = [
            "flex min-h-[5.25rem] w-full items-start gap-3 rounded-xl border p-3 text-left transition",
            active
          ...
          if (disabled) {
            return (
              <div key={step.key || step.label} className={className}>
                {content}
              </div>
            );
          }
          ...
          if (step.href) {
            return (
              <Link key={step.key || step.label} to={step.href} className={className}>
                {content}
              </Link>
            );
          }
